...
{
  private static final Logger logger = Logger.getLogger(LocalConfigHandler.class);

  private final String local_xml = System.getProperty("user.dir")+"/conf/swift-local.xml";
  private final String local_xsd = System.getProperty("user.dir")+"/conf/swift-local.xsd";
  ...
  public void loading()
  {
    ...
    for(Element instance : instanceNodes){
      ...
      if (timerNode != null) {
        String starttimevalue = timerNode.selectSingleNode(start_time_node).getText();
        String intervalvalue = timerNode.selectSingleNode(interval_node).getText();
        Long interval = null;
        if (StringUtil.isNullOrBlank(intervalvalue) || intervalvalue.equalsIgnoreCase("0")) {
          interval = null;
        }else if (!CalculateUtil.isNumeric(intervalvalue)) {
          interval = Long.valueOf(CalculateUtil.parseExp(intervalvalue));
        }else{
        ...
        if (flag) {
          ...
          if(stime.before(new Date())){
            logger.error("swift-local.xml file configuration error : instance ["+idvalue+"] starttime have expired");
            System.exit(1);
          }
        } else {
          logger.error("swift-local.xml file configuration error : <starttime> of instance id="+idvalue);
          System.exit(1);
        }
      ...
      if(proxyNode != null && !proxyNode.getStringValue().trim().equals("")){
        String[] proxys = proxyNode.getStringValue().trim().split("\\|");
      ...
      if(seedsNode == null ){
        logger.error("swift-local.xml file configuration error : <instance> has not <seeds> element of instance id="+idvalue+".");
        System.exit(1);
      }
      ...
      if (inseeds == null && outseeds == null) {
        logger.error("swift-local.xml file configuration error : <seeds> has not children elements of instance id="+idvalue+".");
        System.exit(1);
      }
      if (inseeds != null && inseeds.hasContent()) {
        ...
        if (inele.hasContent()) {
          ...
          if (!StringUtil.isNullOrBlank(inseedsvalue)) {
            if (inseedsvalue.equalsIgnoreCase("all")) {
              ...
              for (int i=0;i<seeds.length;i++) {
                ...
                nseeds[i] = seed;
                logger.info("instance ["+idvalue+"] loading inside seed ["+seed+"]");
              }
          ...
          Constants.inside_delay_map.put(idvalue, xmlobj.getIndelay());
          ...
          if (!StringUtil.isNullOrBlank(useProxy)) {
            ...
            if (useproxy && (proxyNode == null || proxyNode.getStringValue().trim().equals(""))) {
              logger.error("swift-local.xml file configuration error : instance id="+idvalue+" can not set use_proxy=true cause " +
                  "<http_proxy> has not contents .");
              System.exit(1);
            }
            xmlobj.setInUseProxy(useproxy);
          }
          Constants.inside_use_proxy_map.put(idvalue, xmlobj.isInUseProxy());
        } else {
          logger.error("swift-local.xml file configuration error : <seeds>-<inside> has not contents of instance id="+idvalue+".");
          System.exit(1);
        }
      }
        ...
        if (outele.hasContent()) {
          ...
          if (!StringUtil.isNullOrBlank(outseedsvalue)) {
            ...
            for (int i=0; i<nseeds.length; i++) {
              ...
              nseeds[i] = seed;
              logger.info("instance ["+idvalue+"] loading outside seed ["+seed+"]");
            }
          ...
          Constants.outside_delay_map.put(idvalue, xmlobj.getOutdelay());
          ...
          Constants.outside_use_proxy_map.put(idvalue, xmlobj.isOutUseProxy());
        } else {
          logger.error("swift-local.xml file configuration error : <seeds>-<inside> has not contents of instance id="+idvalue+".");
          System.exit(1);
        }
      ...
      if(storageNode == null){
        logger.error("swift-local.xml file configuration error : <instance> has not <storage> element of instance id="+idvalue+".");
        System.exit(1);
      }
...
//        }
      ...
      if (!StringUtil.isNullOrBlank(syncDomain)) {
        ...
        if (xmlobj.isSync()) {
          Constants.sync_domain_map.put(idvalue, xmlobj.getSyncDomain());
        }
      }
      ...
      if(dbNode != null){
        xmlobj.setDbDriver(dbNode.selectSingleNode(driver_node).getText().trim());
        xmlobj.setDbUrl(dbNode.selectSingleNode(url_node).getText().trim());
        xmlobj.setDbUsername(dbNode.selectSingleNode(user_name_node).getText().trim());
        xmlobj.setDbPassword(dbNode.selectSingleNode(password_node).getText().trim());
        xmlobj.setDbPoolSize(Integer.valueOf(dbNode.selectSingleNode(pool_size_node).getText().trim()));
      }
