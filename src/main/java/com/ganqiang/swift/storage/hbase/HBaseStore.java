...
public class HBaseStore implements Storable {
	...
	public <Store> Result readOne(Store... s) {
		...
		try {
			...
			logger.error("Cannot to excute readOne method by hbase  rowkey: "
					+ rowKey, e);
		} finally {
			try {
				if (table != null) {
	...
	public <Store> void writeBatch(Store... s) {
		...
		try {
			...
			for (Result r : results) {
				Put p = new Put(Bytes.toBytes(StringUtil.generateID(r.getUrl())));
				p.add(Bytes.toBytes(columnfamily), Bytes.toBytes("NAME"),
						Bytes.toBytes(r.getName()));
				p.add(Bytes.toBytes(columnfamily), Bytes.toBytes("PLATFORM"),
						Bytes.toBytes(r.getPlatform()));
				if (!StringUtil.isNullOrBlank(r.getBorrower())) {
					p.add(Bytes.toBytes(columnfamily),
							Bytes.toBytes("BORROWER"),
							Bytes.toBytes(r.getBorrower()));
				}
				p.add(Bytes.toBytes(columnfamily), Bytes.toBytes("YEAR_RATE"),
						Bytes.toBytes(r.getYearRate().toString()));
				if (r.getDayRate() != null) {
					p.add(Bytes.toBytes(columnfamily),
							Bytes.toBytes("DAY_RATE"),
							Bytes.toBytes(r.getDayRate().toString()));
				}
				if (!StringUtil.isNullOrBlank(r.getRepayLimitTime())) {
					p.add(Bytes.toBytes(columnfamily),
							Bytes.toBytes("REPAY_LIMIT_TIME"),
							Bytes.toBytes(r.getRepayLimitTime()));
				}
				if (r.getProgress() != null) {
					p.add(Bytes.toBytes(columnfamily),
							Bytes.toBytes("PROGRESS"),
							Bytes.toBytes(r.getProgress().toString()));
				}
				if (!StringUtil.isNullOrBlank(r.getStatus())) {
					p.add(Bytes.toBytes(columnfamily), Bytes.toBytes("STATUS"),
							Bytes.toBytes(r.getStatus()));
				}
				if (!StringUtil.isNullOrBlank(r.getRemainTime())) {
					p.add(Bytes.toBytes(columnfamily),
							Bytes.toBytes("REMAIN_TIME"),
							Bytes.toBytes(r.getRemainTime()));
				}
				if (!StringUtil.isNullOrBlank(r.getRepayMode())) {
					p.add(Bytes.toBytes(columnfamily),
							Bytes.toBytes("REPAY_MODE"),
							Bytes.toBytes(r.getRepayMode()));
				}
				if (r.getRepayPerMonth() != null) {
					p.add(Bytes.toBytes(columnfamily),
							Bytes.toBytes("REPAY_PERMONTH"),
							Bytes.toBytes(r.getRepayPerMonth().toString()));
				}
				if (r.getTotalNum() != null) {
					p.add(Bytes.toBytes(columnfamily),
							Bytes.toBytes("TOTAL_NUM"),
							Bytes.toBytes(r.getTotalNum().toString()));
				}
				if (!StringUtil.isNullOrBlank(r.getCategory())) {
					p.add(Bytes.toBytes(columnfamily),
							Bytes.toBytes("CATEGORY"),
							Bytes.toBytes(r.getCategory()));
				}
				if (r.getRemainMoney() != null) {
					p.add(Bytes.toBytes(columnfamily),
							Bytes.toBytes("REMAIN_MONEY"),
							Bytes.toBytes(r.getRemainMoney().toString()));
				}
				if (!StringUtil.isNullOrBlank(r.getDetailDesc())) {
					p.add(Bytes.toBytes(columnfamily),
							Bytes.toBytes("DETAIL_DESC"),
							Bytes.toBytes(r.getDetailDesc()));
				}
				if (!StringUtil.isNullOrBlank(r.getReward())) {
					p.add(Bytes.toBytes(columnfamily), Bytes.toBytes("REWARD"),
							Bytes.toBytes(r.getReward()));
				}
				if (!StringUtil.isNullOrBlank(r.getAgency())) {
					p.add(Bytes.toBytes(columnfamily), Bytes.toBytes("AGENCY"),
							Bytes.toBytes(r.getAgency()));
				}
				if (!StringUtil.isNullOrBlank(r.getStartTime())) {
					p.add(Bytes.toBytes(columnfamily),
							Bytes.toBytes("START_TIME"),
							Bytes.toBytes(r.getStartTime()));
				}
				if (!StringUtil.isNullOrBlank(r.getEndTime())) {
					p.add(Bytes.toBytes(columnfamily),
							Bytes.toBytes("END_TIME"),
							Bytes.toBytes(r.getEndTime()));
				}
				if (!StringUtil.isNullOrBlank(r.getSecurityMode())) {
					p.add(Bytes.toBytes(columnfamily),
							Bytes.toBytes("SECURITY_MODE"),
							Bytes.toBytes(r.getSecurityMode()));
				}
				if (!StringUtil.isNullOrBlank(r.getCreditRating())) {
					p.add(Bytes.toBytes(columnfamily),
							Bytes.toBytes("CREDIT_RATING"),
							Bytes.toBytes(r.getCreditRating()));
				}
				if (!StringUtil.isNullOrBlank(r.getAvatar())) {
					p.add(Bytes.toBytes(columnfamily), Bytes.toBytes("AVATAR"),
							Bytes.toBytes(r.getAvatar()));
				}
				if (r.getCreateTime() != null) {
					p.add(Bytes.toBytes(columnfamily), Bytes
							.toBytes("CREATE_TIME"), Bytes.toBytes(DateUtil
							.dateToStr(r.getCreateTime())));
				}
				list.add(p);
			}
			table.put(list);
