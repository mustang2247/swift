...
public class SqlBuilder {
  ...
  private static final Logger logger = Logger.getLogger(SqlBuilder.class);
	...
	public void buildSqls(DBStore dbstore, List<Result> list, SiteType type, String instanceid){
	  String thriftServer = "";
    ...
    for(Result result : list){
      //source
      Result dbresult = dbstore.readOne("select * from project where PLATFORM='"+platform+"' " +
          "and MONEY='"+result.getMoney()+"' and NAME='"+result.getName()+"' " +
...
//          "and YEAR_RATE='"+result.getYearRate()+"'"); //year_rate可能为空
      ...
      if(dbresult == null){
        ...
        updateSqls.add(updatesql + buildUpdateSql(result,dbresult.getId()));
        results.add(result);
      }

      //stat
      Result dbdayresult = dbstore.readOne("select * from day_project where PLATFORM='"+platform+"' " +
          "and MONEY='"+result.getMoney()+"' and NAME='"+result.getName()+"' " +
          "and URL='"+result.getUrl()+"' and CREATE_TIME like '" +DateUtil.getCurrentDay()+ "%'");
      if (dbdayresult == null) {
        insertDaySqls.add(insertdaysql + buildInsertSql(result));
      }else if(result.isRequireDayStatUpdate(dbdayresult)){
        updateDaySqls.add(updatedaysql + buildUpdateSql(result,dbdayresult.getId()));
      }
    }
	...
	private void sendIndexDate(List<Result> list, String thriftServer){
      ...
      jsonObject.put("create_time", DateUtil.dateToStr(result.getCreateTime()));
      jsonObject.put("update_time", result.getUpdateTime() != null?DateUtil.dateToStr(result.getUpdateTime()):null);
	...
	private String buildInsertSql(Result result){
		String sql = " '"+StringUtil.generateID(result.getUrl())+"','"+result.getPlatform()+"','"+result.getUrl()+"','"+result.getSite()+"','"+result.getName()+"',";
	...
	private String buildUpdateSql(Result result,String id){
		String sql = "BORROWER='"+result.getBorrower()+"',REPAY_LIMIT_TIME='"+result.getRepayLimitTime()+"',";
    if(result.getProgress() == null){
      sql += "PROGRESS="+null+",";
    }else{
    ...
    if(result.getStatus() == null){
      ...
      sql += "STATUS='"+result.getStatus()+"',";
		...
		sql += "UPDATE_TIME='"+DateUtil.dateToStr(new Date())+"' where id='"+id+"' ";
		return sql;
	}
