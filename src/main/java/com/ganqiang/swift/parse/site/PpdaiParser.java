	...
	public List<Result> parse(Event event) {
		...
		for (FetchedPage fetchedPage : fplist) {
			...
			logger.info("Worker [" + Thread.currentThread().getName()
					+ "] --- [PpdaiParser] begin parse from [" + url + "].");
			...
			if (!StringUtil.isNullOrBlank(result.getAvatar())
					&& result.getAvatar().contains("App_Themes")) {
				result.setAvatar(null);
			}
			String filename = FileUtil.downloadAvatar(instanceid,
					SiteType.PPDAI, path, pageid, logo, result.getAvatar());
			...
			result.setName(name);
			String money = doc.select("span[id=TotalAmount]").text()
			...
			if (!els.isEmpty() && els.first() != null) {
				...
				if (result.getRepayMode().contains("每月还款")) {
					result.setRepayPerMonth(Double.valueOf(repayMode[1]
			...
			if (!StringUtil.isNullOrBlank(remainTime)) {
				if (remainTime.contains("结束时间")) {
					result.setEndTime(remainTime.split("结束时间：")[1].replaceAll(
							"/", "-") + " 00:00:00");
				} else {
			...
			String process = doc.select("div[id=progress]").text().trim()
					.replaceAll("%", "");
			...
			String remainMoney = doc.select("span[id=NeedAmount]").text()
					.trim().replaceAll(",", "").replaceAll("¥", "");
			if (!StringUtil.isNullOrBlank(remainMoney)) {
				result.setRemainMoney(Double.valueOf(remainMoney));
			}
			Elements eel = doc
					.select("table[class=float_l] > tbody > tr > td > span");
			if (!eel.isEmpty() && eel.size() == 4) {
				String totalnum = eel.get(3).text().split("\\|")[0].replaceAll(
						"总投标数：", "");
			...
			results.add(result);
			logger.info("Worker [" + Thread.currentThread().getName()
					+ "] --- [PpdaiParser] end parse from [" + url + "].");
		}
