  ...
  public List<Result> parse(Event event){
    ...
    try {
      for(FetchedPage fetchedPage : fplist){
        ...
        logger.info("Worker [" + Thread.currentThread().getName() + "] --- [JimuboxParser] begin parse from ["+url+"].");
...
//        String uptext = up.text().toString();
        ...
        temp = temp.split("年化利率")[1];
        String yr = temp.split("资金用途")[0].trim();
        if (yr.contains("+")){
          String[] yrs = yr.split("\\+");
          result.setYearRate(Double.valueOf(yrs[0].replaceAll("%", "")));
        ...
        if (money.contains("万")) {
          result.setMoney(Double.valueOf(money.replaceAll("万元", "").trim()) * 10000);
        } else {
        ...
        if (risk.contains("担保方")) {
          String agency = risk.split("担保方 ")[1].split(" 所有担保项目 ")[0].trim();
          result.setAgency(agency);
        }
        
        if (risk.contains(" 担保情况 ")) {
          String securityMode = risk.split(" 担保情况 ")[1].split(" 反担保情况 ")[0].split("提供")[1].split("。")[0].trim();
        ...
        if(!estatus.isEmpty()){
          ...
          Elements p = doc.select("div[style*=position:absolute;width:50px;height:50px;top:0;left:0;text-align:center;line-height:50px]");
          if (!p.isEmpty()) {
            result.setProgress(Double.valueOf(p.text().replaceAll("%", "")));
            result.setStatus(Constants.status_tbz);
            String remainMoney = doc.select("p[style=margin-bottom:0px;] span[class=important]").text().replaceAll("\\,", "");
            result.setRemainMoney(Double.valueOf(remainMoney));
            String remainTime = doc.select("p[style=margin-bottom:0] small[class=muted]").text().replaceAll("剩余时间", "").trim();
            result.setRemainTime(remainTime.replaceAll(" ", ""));
          }
        }
        
        String catagory = doc.select("span[class=tag]").text().trim().replaceAll(" ", "");
        result.setCategory(catagory);
        
        String month = doc.select("span[class=data-tips]").get(1).text().trim().replaceAll(" ", "");
        ...
        String html = HttpHelper.getContentFromUrl(homepage + "Project/GetInvest?take=1000&skip=0&id=" + url.split("Index/")[1]);
        JSONArray json = new JSONArray(html);
        

        result.setTotalNum(json.length());
        
        Elements crele = doc.select("div[class=grade-card] dl[class=dl-horizontal]");
        if (!crele.isEmpty()) {
        ...
        logger.info("Worker [" + Thread.currentThread().getName() + "] --- [JimuboxParser] end parse from ["+ url +"].");
      }
    } catch (Exception e) {
      logger.error("Worker [" + Thread.currentThread().getName() + "] --- [JimuboxParser] execute failure. ",e);
    }
